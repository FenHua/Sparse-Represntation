function[trainLabel,testLabel,trainSample,testSample,numClass]=getTrainTestData(datasetPath,fileExt,imgSide,percentTraining,normalizationFlag)
imgSampleDir=dir([datasetPath,'/*.',fileExt);
numSample=length(imgSampleDir);
sampleAll=zeros(2,numSample);
imResizedHeight=imgSide;
imResizedWidth=imgSide;
sampleImgAll=zeros(imgResizedHeight*imgResizedWidth,numSample);
%load all images
for i=1:numSample
    [sampleAll(1,:),sampleAll(2,i)]=getLabel(imgSampleDir(i).name);
    Img=imread([datasetPath,'/',imgSampleDir(i).name]);%load([datasetPath,'/',imgSampleDir(i).name])
    if ndims(Img)==3
    ...
    img=adapthisteq(img);
    img=imresize(img,[imgResizedHeight,imResizedWidth],'bicubic');
    sampleImgAll(:,i)=img(:);
end
...
for indClass=1:numClass
    ...
    selectIndexes=randperm(numSampleInClass);
    trainIndex(indSampleInClass(selectedIndexes(1:numTrainSampleInClass)))=1;
end
trainIndex=logical(trainIndex);%converts numeric input A into an array of logical values.
