...
for r=1:repeat
    ...
    onesvecs=full(ind2vec(trn.y)); % preparing clssification target
    n_test=size(tst.X,2); % number of tets data to be classified
    ...
    % start classifcation
    for iter=1:n_test
        % looping through all the test samples
        ytilde=tst.X(:,iter);
        ytilde=ytilde/norm(ytilde);% normalizing the test samples
        if strcmp(solver,'omp')
